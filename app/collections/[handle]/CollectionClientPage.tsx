...
export default function CollectionClientPage({
  ...
  // Data states
  const [currentCollection] = useState<Collection | null>(initialCollection);
  ...
  // Update URL if collection select filter changes
  useEffect(() => {
    if (selectedCollection !== handle) {
      router.push(`/collections/${selectedCollection}`);
    }
  }, [selectedCollection, handle, router]);

  // Sync state if search query URL param updates
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Filter by Bike Fitment (dropdown state or active garage bike)
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((p) => isProductMatchingQuery(p, q));
    }
    ...
    setVisibleCount(24);
  }, [products, garageBike, selectedMaker, selectedModel, selectedYear, searchQuery, selectedCategory, selectedBrand, priceRangeFilter, sortBy]);
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-4xl font-headings font-extrabold tracking-tight uppercase">
              {currentCollection ? currentCollection.title : "Performance Collection"}
            </h1>
        ...
        <main className="grid grid-cols-1 gap-8">
          <div className="bg-white border border-brand-border rounded-xl p-5 shadow-lg space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              {/* Switch Collection Dropdown (First) */}
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white border border-brand-border rounded-xl">
              ...
              <h3 className="font-headings font-extrabold text-lg text-brand-primary uppercase">NO COMPATIBLE UPGRADES</h3>
              <p className="text-brand-muted text-xs font-body max-w-sm mx-auto mt-2 leading-relaxed">
                Adjust your filters or clear your garage compatibility profile to browse other segments of this collection.
              </p>
              {hasActiveFilters && (
                <button
                  onClick={handleResetFilters}
